...
struct OxygenDepletionModel{FT, W} <: AbstractContinuousFormBiogeochemistry
    ...
    function OxygenDepletionModel(initial_photosynthetic_slope::FT,
                                     Iopt::FT,
                                     alphaI::FT,
                                     betaI::FT,
                                     gammaD::FT,
...
@inline LimT(T) = exp(0.0663*( T -0.)) #for Arctic (Moore et al.,2002; Jin et al.,2008) 
...
@inline POM_decay_denitr(r_pom_nut_nut,POM,OXY,O2_suboxic,NUT) = r_pom_nut_nut*POM * F_subox(OXY,O2_suboxic) * F_ox(NUT,0.01)  
...
@inline DOM_decay_denitr(r_dom_nut_nut,DOM,OXY,O2_suboxic,NUT) = r_dom_nut_nut*DOM * F_subox(OXY,O2_suboxic) * F_ox(NUT,0.01)  
#! depends on NUT (NO3+NO2) and DOM (NH4+Urea+"real"DON) ! depends on T ! stops at NUT<0.01 
...
@inline function (bgc::OXYDEP)(::Val{:NUT}, x, y, z, t, NUT, PHY, HET, POM, DOM, OXY, T, PAR)
    ...
    return (RespPhy(r_phy_nut,PHY) + RespHet(r_het_nut,HET) + DOM_decay_ox(r_dom_nut_oxy,DOM) 
           ...
           - NtoN * (POM_decay_denitr(r_pom_nut_nut,POM,OXY,O2_suboxic,NUT) 
...
@inline function (bgc::OXYDEP)(::Val{:PHY}, x, y, z, t, NUT, PHY, HET, POM, DOM, OXY, T, PAR)
    ...
    return (GrowthPhy(Max_uptake,PAR,α,T,Knut,NUT,PHY,Iopt)  - GrazPhy(r_phy_het,Kphy,PHY,HET) 
            - RespPhy(r_phy_nut,PHY) - MortPhy(r_phy_pom,PHY) - ExcrPhy(r_phy_dom,PHY)
            )
end
...
@inline function (bgc::OXYDEP)(::Val{:DOM}, x, y, z, t, NUT, PHY, HET, POM, DOM, OXY, T, PAR)
    ...
    return ((1.0-Uz) * Hz * (GrazPhy(r_phy_het,Kphy,PHY,HET) + GrazPOM(r_pom_het,Kpom,POM,HET)) 
            + ExcrPhy(r_phy_dom,PHY) - DOM_decay_ox(r_dom_nut_oxy,DOM) + Autolys(r_pom_dom,POM)
            + POM_decay_denitr(r_pom_nut_nut,POM,OXY,O2_suboxic,NUT) 
           )
...
summary(::OXYDEP{FT, NamedTuple{K, V}}) where {FT, K, V} = string("OxygenDepletionModel{$FT} model, with $K sinking")
show(io::IO, model::OXYDEP{FT}) where {FT} = print(io, string("OxygenDepletionModel{$FT} model \n",
...
adapt_structure(to, oxydep::OXYDEP) = 
OxygenDepletionModel(adapt(to, oxydep.initial_photosynthetic_slope),
